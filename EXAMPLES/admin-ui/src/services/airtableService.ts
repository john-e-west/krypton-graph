...
export const airtableService = {
  ...
  async getOntology(id: string) {
    const record = await base('Ontologies').find(id);
    return {
      id: record.id,
      name: record.get('Name'),
      domain: record.get('Domain'),
  ...
  async getEntitiesByOntology(ontologyId: string) {
    return this.getEntities(ontologyId);
  },

  async updateEntity(id: string, data: any) {
    await base('EntityDefinitions').update(id, {
      'Entity Name': data.name,
      'Entity Class': data.type,
      'Properties JSON': JSON.stringify(data.properties || {}),
      'Validation Rules': JSON.stringify(data.validationRules || {}),
  ...
  async getEdges(ontologyId: string) {
    ...
    return records.map(record => ({
      ...
      sourceId: record.get('Source Entity'),
      targetId: record.get('Target Entity'),
      properties: JSON.parse((record.get('Properties JSON') as string) || '{}'),
  ...
  async getEdgesByOntology(ontologyId: string) {
    return this.getEdges(ontologyId);
  },

  async createEdge(data: any) {
    const record = await base('EdgeDefinitions').create({
      'Edge Name': data.name,
      Ontology: data.ontologyId ? [data.ontologyId] : undefined,
      'Edge Class': data.type || 'default',
  ...
  async updateEdge(id: string, data: any) {
    await base('EdgeDefinitions').update(id, {
      'Edge Name': data.name,
